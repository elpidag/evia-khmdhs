...
def test_real_db_counts(conn):
    ...
    assert conn.execute("SELECT COUNT(*) FROM contract_extension_acts").fetchone()[0] == 470  # +4 since the 2026-08-29 sweep
    assert conn.execute(
    ...
    assert kinds == {"extension": 111, "extension_partial": 358, "extension_refused": 1}
...
def test_real_db_scope_of_the_grant(conn):
    ...
    assert rows == {
        ("extension", "area"): 28, ("extension", "stage"): 1, ("extension", "whole"): 21,
        ("extension", "—"): 61,
