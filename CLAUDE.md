...
that day, +lot 4Α on 2026-09-01): 346 records, **254 in scope; the Atlas
analytics basis is stated net €633,588,292.66** (€785,649,482.93 gross) —
since 2026-08-29
...
II 59 · III 79 · IV 48 · **V-PLUS 20** · ΕΣΑ 9 · restoration 13; effective
retired 2026-08-03, payments their own layer: 905 orders / €586.0M paid
gross (5 Diavgeia-only); 290 completion acts, 470 extension acts; 157
in-scope contractors, all with a curated seat and display name; 62
joint ventures (48 with members, 98 links); categories dasotexnika 162 /
...
137 study & works / 103 works; 404 work-theme links over 210 contracts;
594 municipality rows / 158 contracts / 223 δήμοι (1 flagged outside
their Π.Ε. — Γορτυνίας on lot 4Β, a pasted clause, by user verdict); 254/254 linked to a forest authority (2 documented
...
- `forest_loader.py` — links every contract to its **Διεύθυνση Δασών /
  ...
  `contract_overrides` pin the 8 reviewed title/items conflicts (per-lot
...
- Deliverables layer (DATA_DECISIONS 2026-08-22): **study 14 /
  ...
  `AreaYears` solid lines per scope from the network nodes' d/dk;
  `StackedYears.svelte` exists, parked — a form reserved for breakdowns
  that PARTITION the contracts); the type taxonomies deliberately stay per-dataset. **Nine contracts cite their
...
- `families_loader.py` — **procurement families read from the contracts'
  ...
  is 134). Coverage: **219/245 in-scope contracts → 134 calls** (222 → 136
  since 2026-09-01, when lots 4Α and 4Β of ANTINERO II — both citing their
  call and award by DATE only, the ΑΔΑΜ established by the award and 4Β's
  declared chain — joined via curated `data/family_curation.json`: rows
  merged BEFORE the inheritance walk, source «curated», an heir's
  «inherited:<ref>», evidence per row, real-DB pinned); the
...
- `scripts/find_antinero_new.py` — **the only discovery of a NEW Anti-nero
  ...
  contracts are its control. Writes a JSON report, never the DB. **Two routes
  it does not run yet** (DATA_DECISIONS 2026-09-01): an AWARD's winner list
  — how lot 4Α (22SYMV011331269: no programme word, no fund code, no declared
  family) was found — and an issuer's Diavgeia acts read for a lot's Α/Α
  ΕΣΗΔΗΣ. (The
...
- Municipality layer (proposals only, curation open — DATA_DECISIONS
  ...
  (FK CASCADE; in the refresh chain after details_loader). **594 rows / 158
  contracts / 223 δήμοι** since the 2026-08-31 tier-C verdicts (+ lot 4Α, 2026-09-01) (the
...
- `scripts/extract_site_candidates.py` — scans cached contract PDFs for
  ...
  1152 px, with `--scroll`); on 2026-09-22 the two FLOWS OF MONEY frames
  (`#flows`, `#dase-flows`) — after which a SCROLL box (`overflow: auto`)
  clips like a hidden one, for text lines and for boxes: an element
  wholly outside its nearest clipping ancestor is not drawn (wholly, not
  by its centre — a list container taller than its box still holds
  visible rows); `--css '<stylesheet>'` injects a style before
  serialising — how a scroll box is RELEASED so a list exports WHOLE
  (`dase_flows_destinations.svg`, `antinero_flows_destinations.svg`: the
  flow frames' `.flow-list` with `.scroll{max-height:none}`, 2026-09-22);
  the story's SIGNED timeline canvas exposes `canvas.__dots` like the
...
| `cpv_nodes.json` | Official EU CPV 2008 names (EN + EL) for the 276 nodes the in-scope Anti-nero contracts AND the live ΔΑΣΕ contracts touch — leaf codes and their division/group/class/category ancestors, each at its true level with its parent (DATA_DECISIONS 2026-08-23, ΔΑΣΕ codes 2026-08-24); built by `scripts/build_cpv_nodes.py` from `data/raw/cpv_2008_ver_2013.xlsx` (TED), reading both DBs; feeds `antinero_cpv_tree` + `dase_cpv_tree` (shared `_cpv_rollup`) / both front pages' CPV CODES trees |
...
| `sponsor_names_en.json` | English display names for the 36 sponsors (DATA_DECISIONS 2026-09-23), keyed by the ranking's display name: the company's own English trade name where it publishes one (ΔΕΗ → PPC, ΔΕΔΔΗΕ → HEDNO, ΑΔΜΗΕ → IPTO …), else the legal form rendered over ELOT; 7 entries `review: true` pending the author. Machine-proposed; used so far only by the sponsored-timeline print export — NO site surface |
...
states: **50,2% of the money — €15,00M — is earned by co-ops working
outside their own Π.Ε.** (since the 2026-08-28 seat correction; it read
37,4% while the seat column held the first contract's Π.Ε.), fire-driven
(Εύβοια imported 85% of its €9,47M, Ηλεία and Ρόδος 100%; biggest flow
Τρίκαλα → Εύβοια €2,99M, then Θεσσαλονίκη → Ηλεία €1,78M).
...
since the same day, user) — 35 px inside-label bars, values at the right,
computed biggest-year lightbulb, and deliberately NO «€ paid» lens
(payment coverage is structurally partial, so a paid series would chart
registry practice — the caveat says so, methodology #dase-dedup);
TOP … CO-OPS is retitled **RANKING OF CO-OPERATIVES** with the computed
top-10-share lightbulb and the canonical-ΑΦΜ + even-split caveat in one
clause; CPV MIX is **CPV CODES** — the declared codes rolled up the CPV
2008 tree in the same `CpvColumns` (`queries_extra.dase_cpv_tree` →
`cpv_tree` on `/api/dase/overview`, all 8 divisions officially named,
pinned by `test_dase_real_db.py::test_cpv_tree_pins`; the whole insurance
division — 66519300-4 AND the 8 contracts on 66500000-5 — verified as
the ΕΦΚΑ tag, said in the lightbulb, the caveat keeps the 2026-08-17
note). `CpvColumns` gained a scale-aware tick step (the fixed 25/50 pair
smeared 40 ticks at n=1,998; Anti-nero still lands on 50), prints a
class count ABOVE its bar when too wide to fit inside, and takes the
...
**FLOWS OF MONEY on /dase since 2026-09-22** (author; DATA_DECISIONS): the
Anti-nero flow frame one dataset over, right after MAP — the SAME
`sections/FlowMap.svelte` with `variant="coops"` (the ΔΑΣΕ green ramp,
«co-operatives» in every key and tip, `/dase/coop/` behind the lens;
`FlowArcs color`, `Bipartite hrefBase/entity`, the `--flow-ink/
-imported/-local` variables on the frame's root — the Anti-nero frame
keeps its black) on `queries_extra.dase_region_flows` →
`/api/dase/flows` (seat Π.Ε. → work Π.Ε. on the even split, the year
dimension, per-work-region origins, the co-op ↔ region edges; the
allocation duo's money in the flow shapes, pinned against it by
`test_dase_flows_pins`); the finding «Only N% … based within the
regional unit» computed from the payload.
...
- **Rebrand «FORESTRY WORKS TRACKER»** (2026-08-12, commits
  ...
  pinned to both bases, `ParallelPipelines.svelte` parked) · NETWORKS OF ACTORS (/authorities, renamed
  2026-08-25; since 2026-08-26 NO hero/KPIs — the page opens with the
  map frame titled NETWORK OF ACTORS (ChartFrame kicker titles in ink,
  key-strip legend), `?show=` mode switch on ONE map at the crew map's frame (640×620,
  k 1.08): authorities ALWAYS #406e55 (green core = also awards ΔΑΣΕ;
  pale = no contracts — registry + directory merged; «ΔΑΣΕ-only»
  measured 0) · co-ops · contractors · all; NO dot outlines
  (stroke="none", DotLayer hot keeps none); the shown network's LIST sits beside the map,
  ...
  with the map hairline, one sentence per line, counts+Σ in the ALL
  lens), the side list under it with column labels, fs-13/fs-14 sizes,
  the panel exactly the map's rendered height, no-contract grey #a6a6a6;
  `coops`+`contractors` on `/api/authorities` via
  `queries_extra.authorities_map_points`, co-op € Σ == the ΔΑΣΕ basis,
  pinned) · Methodology in
...
- **Flow charts live on the Anti-nero page** since 2026-08-20 (user): the
  ...
  **Front-page order since 2026-08-23 (user):** ALLOCATION OF FUNDING ·
  ...
  TIMELINE · CPV CODES (since 2026-08-23 the declared codes rolled up the
  CPV's own tree — `CpvColumns.svelte`: one vertical column per division
  (English names only, code numbers muted with a «CPV» prefix, axes
  named), a click SPLITTING the division IN PLACE into thinner black
  class columns while the others stay grey — compressing to 66 px first,
  the smallest stepping aside only when the room is short (said on the
  axis) — a crumb line with the full name and «✕ close», counts white
  inside tall bars, a key beneath that opens into codes;
  `CpvTree.svelte` parked; names from
  `khmdhs/data/cpv_nodes.json` built by
  `scripts/build_cpv_nodes.py` from the TED workbook in `data/raw/`,
  distinct-contract counts that overlap; `queries_extra.antinero_cpv_tree`
  → `cpv_tree`, pinned) — STUDY COSTS is PARKED since 2026-08-23 (user:
...
- **/anadohoi** (sponsor dataset analysis; order since 2026-08-25:
  ...
  answering several fires attaches to EACH — curated `fire_events`
  lists on the 2 composite acts (Maxima 6ΟΗ7, ΔΕΔΔΗΕ 9ΕΘΠ), new
  `act_front` scar basis, 3 new events (Κουβαράς–Σαρωνίδα, Λουτράκι,
  Αίγιο; Chios split into its June/August 2025 fires) — 20 lanes / 69
  dots, lag = fire → ITS OWN first act, median 53,5 d, pinned; a
  multi-fire act's dots share their date and are JOINED by a thin
  vertical tie — the UpSet convention, hover lights the whole act;
  the legend is the TIMELINE panel's organisation — schematic + status
  column + notes in one tinted strip; dots at TRUE dates (only
  same-day pairs sit 6,5 px apart, tie member pinned, said in the
  legend); a CONTEXT STRIP below brushes a period — the chart
  rescales, month rules appear, double-click resets; hover grows an
  act's dots and inks its tie, never recolours (black = revoked);
  fire NAMES alone on the left column — the flame's position carries
  the date),
  ...
  HELLENiQ, mixed-script «ΕRΕΝ» homoglyphs; in `--c-anadohoi` since
  2026-08-25, `valuesRight`) beside — RIGHT half of the pair, the
  grouped frame leads on the LEFT — **WHO THE SPONSORS ARE** (`SponsorGroups.svelte`, the
  same 36 sponsors gathered into the 12 kinds of business of curated
  `sponsor_groups.json` — one bar per group, clicking it opens its member
  companies underneath on the same scale; the € reconcile to the ranking's
  own Σ and the projects with no stated sum are counted beside them, never
  dropped. TWO LENSES (`?sg=`, the CONTRACT TYPE pair): «€ committed» /
  «number of projects», each sorting by its own measure — the count lens
  states a number and nothing else, and the three €0 groups stop being
  blank rows. The two frames are a PAIR since the same day — equal
  halves in `.rankpair`, `minmax(0,1fr)` because the rows are nowrap —
  and the group chart wears **BarH's inside mode verbatim** (user,
  same day, after a heavier-lettered stacked drawing was rejected):
  35 px group bars, names INSIDE in white fs-13 CAPS (fourth round —
  the measuring spans wear the same transform) with BarH's two-line
  clamp, values at the row's RIGHT edge in a `.tail` column
  (note · value · caret, member € under group €), 26 px pale member
...
- **Detail-page template** (2026-08-17, user mockups; refined 2026-08-16
  ...
  **3 rivers since 2026-08-24**, when Σπερχειός joined for ΨΖ3Ψ4653Π8-5Β2
  ...
  map slot (since 2026-08-25 BOTH frame the WHOLE containing Π.Ε. of
  sites/scars + the project's own `pes` via geoContains, reduced by
  `useGeo.nearParts` since 2026-08-26 so a unit's far-flung island part
  (Π.Ε. Ρόδου carries Καστελλόριζο, 1,3° east) never sets the
  window — the same reduction guards PaperMap's `fitPes`, the region
  zoom and the front-page drill, pinned by `maps/nearParts.test.ts` —
  Greece-anchored at the border, multi-regional shows every unit, the
  2026-08-16 scar-bbox window only the fallback — BOTH draw the
  neighbours context land + the dashed Greek land border in PaperMap's
  conventions, and BOTH carry
  unconditional +/−/⌂ zoom/drag controls, the map-svg dress reset on
  the button glyphs; the contract pages' PaperMaps are interactive with
  the zoom stack below the MAP/DIAGRAM switch, fitPad 0.15) (height 460;
  sponsored-overview palette — #f2f2f2 sea, #fff
...
15. **Title vs items-text authority conflicts** — per-lot 2026 contracts
    ...
    the 8 reviewed cases are pinned in `contract_overrides` with evidence.
