...
| `cpv_nodes.json` | Official EU CPV 2008 names (EN + EL) for the 238 nodes the in-scope contracts touch — leaf codes and their division/group/class/category ancestors, each at its true level with its parent (DATA_DECISIONS 2026-08-23); built by `scripts/build_cpv_nodes.py` from `data/raw/cpv_2008_ver_2013.xlsx` (TED); feeds `antinero_cpv_tree` / the front page's CPV CODES tree |
