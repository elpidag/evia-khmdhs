...
2026-08-20 party corrections — 149 geocoded, 126 with a ΓΕΜΗ profile),
...
| `consortium_members.json` | **Who is behind each joint venture** (DATA_DECISIONS 2026-08-20): all 54 in-scope κοινοπραξίες (identified by ΓΕΜΗ legal form «Κοινοπραξία» ∪ registry name — the form alone catches 6 named after their job), each with its members' ΑΦΜ, the document they were read from and the verbatim sentence. **32 have curated members (65 links, 48 firms, 21 of them unknown to the programme otherwise); 22 are `members_documented: false`** — their titles name firms but no document states membership, and a name is not evidence. Traps encoded: the person signing for a member company is not a member, and another venture of the same firms is not either. Proposals from `scripts/extract_consortium_members.py` (+ `consortium_curator.html`); every verdict the user's, 19 reviewed one at a time. Loaded by `khmdhs.consortium_loader` into `consortiums` + `consortium_members` (validation REFUSES a member that is itself a venture, a self-listing, a non-in-scope venture or a flag/list mismatch); in the refresh chain after municipalities_loader |
...
rides beside it. **A second RANKING VIEW** (`?rank=firm` on `/`) attributes the same money to the firms BEHIND the ventures: `queries_extra.antinero_member_firms` replaces a venture that has curated members with them and splits its € evenly (whole cents), leaves everything else alone, and sums to the SAME €622.534.181,72 — 151 names become 144, Τ&Τ ΚΑΤΑΣΚΕΥΕΣ moves 8th→3rd, and the 22 undocumented ventures' €72,8M sits identically in both views (said in the caveat, never hidden). Facts for the copy ship as `consortiums` on `/api/antinero/overview` — never hardcoded. Anti-nero has exactly ONE joint contract left
(24SYMV016018183, an ένωση with no ΑΦΜ of its own): the other joint ventures
sign as a κοινοπραξία that holds its own ΑΦΜ and is one contractor. Where the
...
## Tests (`tests/`, 511 passing — `.venv/bin/python -m pytest`; plus `cd atlas && npm test` for the 105 frontend units)
